...
class NSScrollViewLoader: UIView {
    
    private let centerView: UIView = UIView()
    private let label = UILabel()
    private var timer: Timer?
    private let messages = ["Loading...", "Still loading...", "Troubleshooting...", "Unable to reach"]
    ...
    func startLoading() {
        ...
        timer?.invalidate()
        timer = nil
        timer = Timer.scheduledTimer(timeInterval: 15, target: self, selector: #selector(self.shuffleLoadingMessage), userInfo: nil, repeats: true)
        
        setupLoading(text: messages.first!)
    }
    
    @objc func shuffleLoadingMessage() {
        
        let messageIndex = messages.index(of: label.text!)
        if messageIndex == messages.count - 2 {
            timer?.invalidate()
            timer = nil
        }
        ...
        UIView.transition(with: label,
                          ...
                          animations: { [weak self] in
                            self?.label.text = self?.messages[messageIndex! + 1]
            }, completion: nil)
    }
